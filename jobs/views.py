...
def all_jobs(request):
    jobs = Job.objects.filter(end_date__gte=date.today()).order_by('-start_date')
    applicant = Applicant.objects.get(user=request.user)
    ...
    if request.method=="POST":
        ...
        if job_title!=None:
            jobs=Job.objects.filter(title__icontains=job_title).filter(end_date__gte=date.today())
